...
- 证书或站点协议热更新后，HTTP/1.1 连接在响应完成后淘汰；HTTP/2 保留进行中的响应，在后续请求上通过 GOAWAY 排空连接，无后续请求时按空闲超时回收。协议不匹配的请求仍返回 `421`，促使客户端重新协商
...
Deep Monitor 重启恢复时，事件数按日志中连续可读取的完整事件重建；已存储字节按 `events.pb` 与会话目录内载荷文件的实际大小重建，包含崩溃遗留的尾帧或未提交载荷，不含索引和会话元数据。原活动会话仍以中止状态保留；因 I/O 错误无法确定的统计保留原值，避免将读取失败误报为零。

HTML `206 Partial Content` 响应保持原始表示，不进行解码、路径重写或工具栏注入。工具栏仅在可确认的 HTML 上下文注入；SVG/MathML 跟踪超过固定深度或标签名长度、混合 HTML 命名空间或出现在 `select` 等特殊结构中，以及 HTML 正文中的 CDATA 标记、未知声明等情况，会保守跳过本响应的工具栏。仅含纯文本或标准 CDATA 的 SVG `title`、`desc` 不受此限制。
...
- 日志统计的域名、路径等维度键拥有独立字符串存储，避免短键连带保留整条长 URL；已有键更新也遵守此约束。
- 统计缓存同时受最多 30 天和 **8 MiB 记账预算**约束，优先淘汰最近最少使用的日期；每项空闲 **5 分钟**后由定时器释放，无需下一次查询触发。超过预算的单日结果仍正常返回，但不缓存。预算采用字符串长度和保守的容器开销估算，不是整个进程的 RSS 硬上限；查询临时对象和仍被读取的快照另计。淘汰后查询会重新扫描日志。
- UA 解析器在首次遇到非空 User-Agent 的统计查询时初始化，随后以紧凑的 radix trie 常驻；无结果的单分支路径合并为边标签，保留原有逐字符跳过规则、优先级和版本解析。
- WAF 关键词匹配的 NFA 在构建后转换为连续、无指针的状态和转移数组，以有界的 32 位索引替代分散对象；沿用原有匹配算法、失败转移和捕获顺序。DFA 构建不变，超出索引范围的输入保留原始表示。

匹配库的固定上游版本、局部改动和等价测试见 [UA 解析器说明](third_party/go-useragent/LOCAL_CHANGES.md) 与 [WAF 自动机说明](third_party/aho-corasick/LOCAL_CHANGES.md)。`task test`、`task test:race` 和 CI 包含这两个子模块的测试。

## Linux 内存与透明大页

Linux 网关在服务初始化前默认通过 `PR_SET_THP_DISABLE` 禁用本进程的透明大页（THP），减少小堆在激进大页合并策略下的 RSS 膨胀。无需增加启动参数；不修改主机全局 THP 设置、GC 参数或 Go 内存软上限。此进程策略也会由网关启动的子进程继承。非 Linux 平台不受影响；内核不支持或限制该调用时，记录警告并继续启动。

需要保留原有大页行为进行性能对照时，可在启动环境设置 `GODEBUG=disablethp=0`；若已有其他 GODEBUG 配置，追加此项即可，例如 `GODEBUG=http2client=0,disablethp=0`。重复的 `disablethp` 项以最后一个有效整数为准；`0` 仅跳过网关的禁用操作，不解除父进程继承的禁用策略。环境变量本身不会被网关改写。

部署后可查看 `/proc/<PID>/smaps_rollup` 的 `Rss`、`Anonymous` 和 `AnonHugePages`，结合控制面的存活堆、运行时管理内存及同负载下的延迟比较效果。启动前已经形成的大页不会被此调用主动拆分；不要将全部大页占用视为可节省内存。`GOMEMLIMIT` 是 Go 运行时软上限，不是进程 RSS 硬上限。
