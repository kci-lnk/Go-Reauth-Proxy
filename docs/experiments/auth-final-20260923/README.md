...
This measures local gateway handler per-operation elapsed time (`ns/op`) and allocation, with parsing prepared outside the loop and a simulated upstream transport; it does not measure process CPU time. It does not measure Linux service throughput, peak RSS, SQLite concurrency, or end-to-end authentication latency. Matched Linux artifacts require their own formal A/B and soak validation.
