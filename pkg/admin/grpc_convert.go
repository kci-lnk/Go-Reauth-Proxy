...
func hostRulesBundleToProto(
	...
	for _, rule := range rules {
		item := &pb.HostRule{
			...
			WebsiteIconPath: rule.WebsiteIconPath,
...
func protoToHostRules(req *pb.HostRules) []models.HostRule {
	...
	for _, rule := range req.GetItems() {
		...
		rules = append(rules, models.HostRule{
			...
			WebsiteIconPath:  rule.GetWebsiteIconPath(),
...
func wafConfigToProto(cfg models.WAFConfig) *pb.WafConfig {
	return &pb.WafConfig{
		...
		ViolationRateLimitEnabled:       cfg.ViolationRateLimitEnabled,
		ViolationRateLimitCapacity:      int32(cfg.ViolationRateLimitCapacity),
		ViolationRateLimitRefillSeconds: int32(cfg.ViolationRateLimitRefillSeconds),
...
func protoToWAFConfig(cfg *pb.WafConfig) models.WAFConfig {
	...
	return models.WAFConfig{
		...
		ViolationRateLimitEnabled:       cfg.GetViolationRateLimitEnabled(),
		ViolationRateLimitCapacity:      int(cfg.GetViolationRateLimitCapacity()),
		ViolationRateLimitRefillSeconds: int(cfg.GetViolationRateLimitRefillSeconds()),
