...
func (h *Handler) proxyFnosPortIconHijackWebSocket(w http.ResponseWriter, r *http.Request, options fnosPortIconHijackWebSocketOptions, targets map[int]string) error {
	...
	// Keep relaying until a peer disconnects, forwarding fails, or the request
	// is canceled. Close both sockets and join every remaining relay before
	// the request's monitor is finalized.
	pendingRelays := 2
	select {
	case err = <-errCh:
		pendingRelays--
...
func buildFnosPortIconHijackWebSocketURL(
	...
	if hostTargetPathMode != "" {
		applyHostReverseProxyPath(&upstreamURL, targetURL, incomingURL, hostTargetPathMode)
	} else {
	...
	if options.rewriteOriginReferer {
		...
		if referer := r.Header.Get("Referer"); referer != "" {
			if ref, err := url.Parse(referer); err == nil {
				...
				if options.hostTargetPathMode != "" {
					applyHostReverseProxyPath(ref, options.targetURL, ref, options.hostTargetPathMode)
				} else {
