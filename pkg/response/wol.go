...
(function(){
  ...
  function shutdownIcon(){var icon=document.createElementNS('http://www.w3.org/2000/svg','svg');icon.setAttribute('viewBox','0 0 24 24');icon.setAttribute('fill','none');icon.setAttribute('stroke','currentColor');icon.setAttribute('stroke-width','2');icon.setAttribute('stroke-linecap','round');icon.setAttribute('stroke-linejoin','round');icon.setAttribute('aria-hidden','true');[['circle','cx','12','cy','12','r','10'],['rect','width','6','height','6','x','9','y','9','rx','1']].forEach(function(spec){var node=document.createElementNS('http://www.w3.org/2000/svg',spec[0]);for(var index=1;index<spec.length;index+=2)node.setAttribute(spec[index],spec[index+1]);icon.appendChild(node)});return icon}
