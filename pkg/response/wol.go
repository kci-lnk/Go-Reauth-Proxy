...
(function(){
  ...
  var relativeTimeFormatter=typeof Intl!=='undefined'&&Intl.RelativeTimeFormat?new Intl.RelativeTimeFormat(document.documentElement.lang||undefined,{numeric:'auto'}):null;
  function relativeTime(value){var timestamp=Date.parse(value);if(!Number.isFinite(timestamp))return new Date(value).toLocaleString();var seconds=(timestamp-Date.now())/1000;var absolute=Math.abs(seconds);if(absolute<45)return labels.justNow;var amount;var unit;if(absolute<3600){amount=Math.round(seconds/60);unit='minute'}else if(absolute<86400){amount=Math.round(seconds/3600);unit='hour'}else if(absolute<604800){amount=Math.round(seconds/86400);unit='day'}else{return new Date(timestamp).toLocaleString()}if(amount===0)return labels.justNow;return relativeTimeFormatter?relativeTimeFormatter.format(amount,unit):new Date(timestamp).toLocaleString()}
  function updateRelativeTimes(){grid.querySelectorAll('[data-checked-at]').forEach(function(node){node.textContent=labels.lastChecked.replace('{time}',relativeTime(node.dataset.checkedAt))})}
  ...
  setInterval(updateRelativeTimes,30000);load();
})();
</script>
</body></html>`
...
func WOLPage(w http.ResponseWriter, r *http.Request) {
	...
	data := wolPageData{
		...
		JustNow:                  i18n.T(locale, "gateway.wol.justNow"),
