...
type Server struct {
	...
	// DisableExtendedConnect omits SETTINGS_ENABLE_CONNECT_PROTOCOL.
	// fn-knock patch: preserve TCP WebSocket fallback for handlers without RFC 9220 support.
	DisableExtendedConnect bool
...
func (s *Server) settings() *settings {
	return &settings{
		...
		ExtendedConnect:     !s.DisableExtendedConnect,
